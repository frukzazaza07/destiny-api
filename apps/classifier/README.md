...
The service never logs raw question text.
...
`scripts/train_model.py` writes `artifacts/classifier.joblib`. The artifact contains
the fitted pipeline, fixed intent-to-domain mapping, and model version
`tfidf-logreg-seed-v1`. The server rejects artifacts with a different version or
taxonomy.
